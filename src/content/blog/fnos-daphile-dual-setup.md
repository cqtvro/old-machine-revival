...
1. **创建虚拟机**：
   ...
   * **操作系统**：选择 Linux（类型可选择 `Debian` 或 `其他 Linux`），具体配置参考下图。
...
4. **正式安装系统**：
   ...
   * **操作**：打开浏览器，输入刚才控制台显示的 IP 地址。进入达菲管理界面后，依次点击 `Settings` -> `System` -> `Install Daphile`，将系统安装到我们刚才设置的 **4GB 虚拟硬盘**上。
...
3. 导航到 **Settings** -> **System** 设置页面，找到 **Media Server** 选项：
   * 将选项设为 **“Bridge external”** (桥接外部媒体服务器)。
...
4. 随后在小主机的 `Settings` -> `Audio Device` 中，勾选并设置您连接的 USB 解码设备即可。
...
* 进入 `Settings` -> `Storage` -> `Network Drives` (网络驱动器) 部分，点击 **“Add new”** (添加项目)。
* 按照以下参数设置：
  * **协议**：选择 **NFS**。
...
* 在控制端的 **“我的音乐 (My Music)”** 里，依次进入 **“浏览音乐文件夹 (Browse Music Folders)”**。
