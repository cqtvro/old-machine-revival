...
const BLOG_POSTS = [
    {
        id: "hivi-d1010mkii-passive",
        title: "【折腾】闲置的惠威 D1010MKII 有源音箱，改成无源音箱，连上数码功放，味道不一样",
        summary: "今天我们来折腾一台旧音箱——闲置多年的惠威 D1010MKII 有源音箱。由于内置功放电路老化出现电流杂音，扔了可惜，于是动手将其彻底掏空改造成无源箱，引入 3.3uF 电容作高音分频，并接入外置数码功放，让老音箱重新焕发生机！",
        category: "audio",
        categoryName: "音频设备",
        date: "2026-05-18",
        readTime: "5 分钟阅读",
        views: 195,
        likes: 42,
        tags: ["有源改无源", "惠威音箱", "硬件DIY", "音频折腾"],
        bannerGradient: "linear-gradient(135deg, #4e342e 0%, #8d6e63 100%)",
        content: `
            <p><strong>前言：</strong>今天我们来折腾一台旧音箱。这是一台闲置多年的惠威 D1010MKII 有源音箱。由于时间太久，功放部分电路老化，出现了一些杂音和电流声。丢了可惜，扔了心疼，于是我决定动手，将其改装成无源音箱。</p>
            
            <p>改装的思路其实非常简单：<strong>拆除内置的功放电路板，将它彻底掏空，只保留扬声器单元（高音和低音），然后重新布线接出。</strong></p>

            <img src="hivi_d1010_front.jpg" alt="惠威 D1010MKII 改造前外观">

            <h2>第一步：掏空旧功放，重整箱体</h2>
            <p>拆开箱体后可以看到，主箱里面塞着功放板和变压器，而副箱里面则是空的。我们要把主箱里的变压器和功放板全部拆除，只保留箱体和喇叭，让它变成和副箱一样的无源结构。</p>
            <p>这样一来，两只箱子就变成了一对双胞胎。</p>
            <p>我们准备了一对双声道接线盒，用来接驳外置功放。为了保护箱体的气密性，接线盒安装的位置必须打胶密封好。</p>

            <img src="potentiometer_mod.jpg" alt="拆卸旧功放旋钮电位器">

            <h2>第二步：喇叭单元连接与电容分频</h2>
            <p>具体的接线方法也很直接。高音喇叭和低音喇叭如果直接并联，声音会混成一团。因此我们必须接入分频器，或者用一个无极电容作为最简单的高音分频。</p>
            <p>这里我使用了一个最简单实用的方案：<strong>高音喇叭正极串联了一个 3.3uF 的无极电容</strong>（作为一个一阶低通分频，过滤掉低频信号，防止烧毁高音单元），低音喇叭则直接直通。</p>
            <p>布线完毕后，把线头焊接到接线柱上，组装好箱体，大功告成。</p>

            <img src="tpa3116_amp_inside.jpg" alt="数码功放板内部结构">

            <h2>第三步：为无源音箱选择合适的功放</h2>
            <p>掏空功放电路后，音箱现在必须要依靠外置的功放来驱动了。我手头正好有一个闲置的数码小功放，使用的是 TPA3116 芯片，虽然体积小巧，但是驱动这台音箱绰绰有余。</p>

            <img src="hp_mini_pc.jpg" alt="HP 小主机播放源">

            <h2>第四步：搭配老机器使用，让声音发挥出新的色彩</h2>
            <p>为了测试实际效果，我找来了一台闲置的 HP 小主机作为播放源，通过 USB 接口连接解码器，再输出到功放，最后驱动音箱。</p>
            <p>整个系统在我的小电脑桌上安顿下来，接通电源，音乐响起的那一刻，我惊呆了：</p>
            <ul>
                <li>原本因为电路老化而存在的杂音、电流声彻底消失了。</li>
                <li>得益于 TPA3116 功放充足的功率，声音变得干净、扎实。</li>
                <li>低音部分有了一定的弹性，不像以前那种发闷、浑浊的感觉。</li>
                <li>高音部分经过电容分频，虽然不算特别细腻，但非常通透，非常适合听一些人声和轻音乐。</li>
            </ul>

            <img src="hivi_setup_final.jpg" alt="最终改造完成的音箱与数码功放/HP主机系统">

            <p>为了进一步改善听感，我还计划给它搭配一个便宜的胆机前级。大家都说“胆机”声音暖，适合人声，这也为下一次折腾埋下了伏笔。</p>

            <hr>

            <h3>🔧 折腾清单：</h3>
            <ul>
                <li>1. 惠威 D1010MKII 有源音箱（闲置）</li>
                <li>2. 3.3uF 无极分频电容 x2 （用于高音分频）</li>
                <li>3. 双孔音箱接线盒 x2</li>
                <li>4. TPA3116 数码功放（闲置）</li>
                <li>5. HP 小主机 + 简易解码器（播放源）</li>
                <li><strong>总花费：约 15 元</strong>（电容和接线盒费用，其余均为闲置设备）</li>
            </ul>

            <blockquote>
                <strong>结语：</strong>“折腾”是一种生活态度。拯救旧设备，不一定需要高大上的技术，有时候一点点动手能力加一些奇思妙想，就能让那些积灰的“电子垃圾”重新焕发光彩。如果你手头也有闲置的旧音箱、旧功放、旧电脑，不妨也动手试试吧，让它们重新唱歌！
            </blockquote>
        `
    ,
